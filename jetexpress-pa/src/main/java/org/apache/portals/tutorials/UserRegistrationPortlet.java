...
import org.apache.portals.tutorials.model.User;
...
public class UserRegistrationPortlet extends GenericPortlet {
    ...
    public void renderForm(RenderRequest request, RenderResponse response)
        ...
        if ("success".equalsIgnoreCase((String) request.getPortletSession()
                .getAttribute("actionStatus"))) {
            PortletURL homeUrl = response.createRenderURL();

            request.setAttribute("homeUrl", homeUrl);
            getPortletContext()
                    .getRequestDispatcher("/WEB-INF/view/tutorials/success.jsp").include(
                    request, response);
            return;
        }
        ...
        if (!"error".equalsIgnoreCase((String) request.getPortletSession()
                .getAttribute("actionStatus"))) {
            request.setAttribute("email", defaultEmail);
        }
        getPortletContext().getRequestDispatcher(
                "/WEB-INF/view/tutorials/registrationForm.jsp").include(request, response);
    }
    ...
    public void registerUser(ActionRequest request, ActionResponse response)
        ...
        //-- method
        request.getPortletSession().setAttribute("user", new User(
    ...
    public void renderPrefs(RenderRequest request, RenderResponse response)
            throws PortletException, IOException {
        getPortletContext()
                .getRequestDispatcher("/WEB-INF/view/tutorials/preferences.jsp").include(
                request, response);
    }
